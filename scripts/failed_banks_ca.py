...
#need to create a reader object
filtered_banks = []

with open("banklist.csv", "r") as localfile:
    headers = localfile.keys() 
    dict_reader = csv.DictReader(localfile, fieldnames = headers) 
    for banks in localfile:
        if headers["State"] == "CA":
...
col_headers = ["Bank Name", "City", "State", "Cert", "Acquiring Institution", "Closing Date", "Fund"]

with open("failed_banks_ca.csv", "w") as newfile:
